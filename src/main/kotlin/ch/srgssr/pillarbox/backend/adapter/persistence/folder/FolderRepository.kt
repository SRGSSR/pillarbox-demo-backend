...
import ch.srgssr.pillarbox.backend.adapter.persistence.media.toPredicate
 ...
 * @param clock The clock media visibility predicates and timestamps are evaluated with.
 */
...
class FolderRepository(
  ...
  override fun Table.encode(
    ...
    builder[FolderTable.createdAt] = clock.now().toUtcOffsetDateTime()
    builder[FolderTable.updatedAt] = clock.now().toUtcOffsetDateTime()
  }
  ...
  override fun encodeOnUpdate(item: Folder): (UpsertBuilder.(UpdateStatement) -> Unit) =
    {
      ...
      it[FolderTable.updatedAt] = clock.now().toUtcOffsetDateTime()
    }

  /**
   * Lists folders within the given scope via the [FolderTable.parentId] column.
   *
  ...
  override suspend fun list(
    scope: FolderScope,
    slice: QuerySlice,
  ): List<Folder> = getAll(slice.limit, slice.offset, filter = scope.toFilter()).toList()

  /**
   * Translates a [FolderScope] into the parent-column filter of a folder listing.
   *
   * @return The `WHERE` predicate builder, or `null` for an unfiltered listing.
   */
  private fun FolderScope.toFilter(): (() -> Op<Boolean>)? =
    when (this) {
      ...
      FolderScope.Unassigned -> {
        { FolderTable.parentId.isNull() }
      }

      is FolderScope.In -> {
        { FolderTable.parentId eq folderId }
      }
    }
   ...
   */
  override suspend fun findAncestors(folderId: String): List<Folder> =
    query(readOnly = true) {
      FolderAncestorView
        .selectAll()
   ...
   */
  override suspend fun findFolderOf(mediaId: String): Folder? =
    query(readOnly = true) {
   ...
   */
  override suspend fun findFoldersOf(mediaIds: Collection<String>): Map<String, Folder> =
    query(readOnly = true) {
      if (mediaIds.isEmpty()) {
        emptyMap()
      } else {
   ...
   */
  override suspend fun isMediaInFolder(
    folderId: String,
    mediaId: String,
  ): Boolean =
    query(readOnly = true) {
   ...
   */
  override suspend fun assignMedia(
    folderId: String,
    mediaId: String,
  ): Unit =
    query {
      FolderMediaTable.upsert(FolderMediaTable.mediaId) {
        ...
        it[FolderMediaTable.addedAt] = clock.now().toUtcOffsetDateTime()
      }
    }
   ...
   */
  override suspend fun removeMediaAssignment(
    folderId: String,
    mediaId: String,
  ): Boolean =
    query {
   ...
   */
  override suspend fun countMediaIn(vararg folderIds: String?): Map<String?, Long> =
    query(readOnly = true) {
   ...
   */
  override suspend fun countSubfoldersOf(folderId: String?): Long =
    query(readOnly = true) {
      FolderTable
        .selectAll()
   ...
   */
  override suspend fun countSubfoldersOf(vararg folderIds: String?): Map<String?, Long> =
    query(readOnly = true) {
      val named = folderIds.filterNotNull().distinct()
      val results = named.associateWith { 0L }.toMutableMap<String?, Long>()
