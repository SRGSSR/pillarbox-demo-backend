package ch.srgssr.pillarbox.backend.application.authz
 ...
 *
 * @property folderGrants Port used to read the grants of a folder chain.
 * @property folderCatalog Port used to resolve the folder of a media item.
 * @property teamCatalog Port used to resolve the team memberships of a user.
 */
class PermissionChecker(
  private val folderGrants: FolderGrants,
  private val folderCatalog: FolderCatalog,
  ...
  suspend fun canWriteFolder(
    ...
    when {
      ...
      folderId == null -> {
        ...
        val grants = folderGrants.findGrantsInChain(folderId)
  ...
  suspend fun canWriteFolders(
    ...
    when {
      ...
      !user.hasAnyRole(setOf(Role.WRITE)) -> {
        ...
        val grantsByFolder = folderGrants.findGrantsInChains(folders.map { it.id })
  ...
  suspend fun canWriteMedia(
    user: User,
    mediaId: String,
  ): Boolean = canWriteFolder(user, folderCatalog.findFolderOf(mediaId)?.id)
  ...
  ): Map<String, Boolean> = canWriteMedia(user, mediaIds, folderCatalog.findFoldersOf(mediaIds))
  ...
  private suspend fun User.teamIdsFor(grants: List<FolderPermission>): Set<String> =
    if (grants.any { it.subject is PermissionSubject.ForTeam }) {
      teamCatalog.findTeamsOf(oidcSub).map { it.id }.toSet()
    } else {
      emptySet()
    }
}
